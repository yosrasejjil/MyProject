...
# Assuming CustomException, logging, save_object, and evaluate_classification_model are custom modules
from src.exception import CustomException
from src.logger import logging
from src.utils import save_object, evaluate_classification_model
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self, X_train, y_train, X_test, y_test):
        try:
            ...
            best_model_name = max(model_report, key=lambda x: model_report[x]["test_metrics"][-1] if model_report[x]["test_metrics"][-1] is not None else 0)
            best_model = models[best_model_name]
            best_model_score = model_report[best_model_name]["test_metrics"][-1]
            ...
            return {
                ...
                "metrics": model_report[best_model_name]["test_metrics"]
            }
